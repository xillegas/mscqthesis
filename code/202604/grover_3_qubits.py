...
"""En resumen, este código es una versión del algoritmo de Grover didáctica, pues está orientado a 3 qubits 
La simulación finaliza mostrando un dibujo del circuito de grover por etapas, y devolviendo una lista 
con la frecuencia de los resultados obtenidos"""
"""
     ┌───┐ ░                 ░ ┌───┐┌───┐          ┌───┐┌───┐      ░ ┌─┐      
q_0: ┤ H ├─░────────■────────░─┤ H ├┤ X ├───────■──┤ X ├┤ H ├──────░─┤M├──────
     ├───┤ ░ ┌───┐  │  ┌───┐ ░ ├───┤├───┤       │  ├───┤├───┤      ░ └╥┘┌─┐   
